<template>
    <div>
        <div class="flow-top-wrapper">
            ...
            <div class="flow-top">订单编号：{{$route.params.ordOrderId}}
                <span class="flow-result" v-if="ordStatus==2">等待收货</span>
                <span class="flow-result" v-if="ordStatus==3">交易完成</span>
            </div>
            <div class="space-bottom"></div>

            <!-- <div class="table" v-if="dataList">
                <div class="table-scroll" v-for="(item,index) in dataList.orderSubList" :class="{afterli:tabActiveIndex==index}" @click="changeTab(index,item.ordOrderId,item.ordStatus)">包裹{{index+1}}</div>
            </div> -->
            <div class="table">
                <div class="table-scroll afterli">包裹1</div>
            </div>
        </div>
        <div class="flow-white"></div>
...
export default {
    ...
    methods: {
        ...
        //     this.getLogisticsData(this.$route.params.ordOrderId);
        ...
        // },
        getSubOrderDetail() {
            client.post(apiConfig.API_SUB_ORDER_DETAIL_BY_ID+"?ordOrderId="+this.$route.params.ordOrderId, {}).then((data) => {
                this.isLoading = false;
                if (data.code === 200) {
        ...
        getLogisticsData() {
            client.post(apiConfig.API_GOODS_LOGISTICS+"?ordOrderId="+this.$route.params.ordOrderId, {}).then((data) => {
                this.isLoading = false;
                if (data.code === 200) {
