    ...
    <div class="content-preload" :class="{show:barList}">
      ...
      <div class="all">
        ...
        <div class="main-box">
          ...
          <div class="middle" v-for="(item,index) in dataRelate" @click="goLink(item)">
            <div class="icons" :style="{background:'url('+rankIcon[index]+') no-repeat center'}" v-if="index<3">
              <div></div>
            </div>
            <div class="icons numbers" v-else>
              <div>{{index+1}}</div>
            </div>
...
export default {
  data() {
    return {
      ...
      all:true,
      rankIcon: [
        require('./images/ic_no1@2x.png'),
        require('./images/ic_no2@2x.png'),
        require('./images/ic_no3@2x.png')
      ],
      currentPage: 1,
  ...
  methods: {
    ...
    //tracker
    goTracker(item){
      tracker.trackData({component_id:item.componentId,fence_id:this.fenceId});
    }
  },
  
  mounted() {
    window.addEventListener('scroll', this.menu);
    tracker.firstRead({ id: 218, title:'',page_name:'榜单页' });
    tracker.scrollCase({ event_name:'榜单页' });
...
.middle {
  ...
  .icons {
    ...
    &.numbers {
      background: url('./images/ic_noother@2x.png') no-repeat center;
