...
if (!client.IS_IOS && /fstop/.test(fsAgent)) {
...
//安卓或ios
const osType = client.IS_IOS == true ? 2 : 1;
//平台类型
...
const postArr = function() {
    if (client.IS_WEIXIN) {
        //微信环境使用openId统计精准用户
        if (localStorage.getItem('trackerOpenId')) {
            startUnique = localStorage.getItem('trackerOpenId');
        }
    }
    return {
        "app_key": "100001",
...
var scrollCase = function(event) {
    ...
    document.addEventListener('touchend', function() {
        ...
        if (eTop - sTop > 20 || sTop - eTop > 20) {
            ...
            client.postData(apiConfig.TRACKER, [scrollEvent]).then((data) => {
                //console.log(data);
                if (data.code != 200) {
                    return
                }
...
const firstRead = function(e) {
    let code = client.getQueryString('code');
    let trackerOpenId = localStorage.getItem('trackerOpenId');
    if(client.IS_WEIXIN && !trackerOpenId && code){
        //微信环境使用openId统计精准用户
        client
        .getData(
        ...
        .then(
            res => {
                if (res.code == 200 && res.data.openid) {
                    localStorage.setItem('trackerOpenId', res.data.openid);
                }
                firstReadFunc(e);
            },
            res => {
...
const firstReadFunc = function(e) {
    firstEvent = postArr();
    ...
    client.postData(apiConfig.TRACKER, [firstEvent]).then((data) => {
        //console.log(data);
        if (data.code != 200) {
            return
        }
...
const trackData = function(e) {
    ...
    client.postData(apiConfig.TRACKER, [clickEvent]).then((data) => {
        if (data.code != 200) {
            return
        }
    }, (data) => {})
